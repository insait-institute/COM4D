...
def load_image(img_path, bg_color=None, rmbg_net=None, padding_ratio=0.1, device='cuda', center: bool = True):
    ...
    if center:
        ...
        base = int(max(w, h) * padding_ratio)
        if w > h:
            left = base; right = base
            top = base; bottom = base + (w - h)  # extra to bottom
        else:
            left = base; right = base + (h - w)  # extra to right
            top = base; bottom = base
        padding_size[0] = left
        padding_size[1] = right
        padding_size[2] = top
        padding_size[3] = bottom
    padded_tensor = F.pad(rgb_image_gpu[:, y:(y+h), x:(x+w)], pad=tuple(padding_size), mode='constant', value=bg_gray)

    return padded_tensor

def prepare_image(image_or_path, bg_color=np.array([1.0, 1.0, 1.0]), rmbg_net=None, padding_ratio=0.1, device='cuda', center: bool = True):
    """Prepare an image (path, PIL, or numpy array) by removing background and returning a PIL.Image."""
    if isinstance(image_or_path, (Image.Image, np.ndarray)) or (isinstance(image_or_path, str) and os.path.isfile(image_or_path)):
        img_tensor = load_image(image_or_path, bg_color=bg_color, rmbg_net=rmbg_net, padding_ratio=padding_ratio, device=device, center=center)
        if isinstance(img_tensor, str):
            raise ValueError(img_tensor)
        img_np = img_tensor.permute(1,2,0).cpu().numpy()
        img_pil = Image.fromarray((img_np*255).astype(np.uint8))
        return img_pil
    else:
        raise ValueError(f"Invalid image input: expected path/PIL/ndarray, got {type(image_or_path)}")
